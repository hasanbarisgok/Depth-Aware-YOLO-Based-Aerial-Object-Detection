...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare YOLO dataset structure under project/dataset.")
    parser.add_argument(
        "--source-root",
        type=Path,
    ...
    for src_split, dst_split in SPLIT_MAP.items():
        ...
        label_src = args.source_root / "Annotations" / "YOLOv8 format" / src_split / "labels"
        ...
        if not label_src.exists():
            raise FileNotFoundError(f"Label source not found: {label_src}")
